...
import { DepartmentItem, DepartmentMember, RapportSoumis, RapportTemplate, UserProfile, CultePresenceRecord } from '../types';
...
import { computeCompartmentPresence, KNOWN_SUNDAYS, getAllSundayDates } from '../utils/presenceUtils';
import { Lock, FileText, Send, UserX, CheckCircle2, AlertCircle } from 'lucide-react';
...
export const DepartmentsView: React.FC<DepartmentsViewProps> = ({
  ...
  // Leadership Guard & Report states
  const [isLeaderGuardOpen, setIsLeaderGuardOpen] = useState(false);
  const [isCreateReportOpen, setIsCreateReportOpen] = useState(false);
  const [targetDeptForReport, setTargetDeptForReport] = useState<DepartmentItem | null>(null);
  ...
  // Sunday dates for presence tracking in Departments
  const allSundays = useMemo(() => getAllSundayDates(presences), [presences]);
  const [selectedSunday, setSelectedSunday] = useState<string>(() => allSundays[0] || KNOWN_SUNDAYS[0]);
  const [presenceSubTab, setPresenceSubTab] = useState<'all' | 'presents' | 'absents'>('all');
  ...
  // Presence status for selected department members
  const deptPresenceStatus = useMemo(() => {
    if (!selectedDept || !selectedDept.membersList || selectedDept.membersList.length === 0) {
      return { presents: [], absents: [], chronicAbsents: [], presenceRatio: 0, matchedRecordsMap: new Map() };
    }
    return computeCompartmentPresence(selectedDept.membersList, presences, selectedSunday, allSundays);
  }, [selectedDept, presences, selectedSunday, allSundays]);
  ...
  return (
    <div className="max-w-6xl mx-auto px-3 sm:px-6 py-6 pb-24 space-y-6">
        ...
        <div className="relative z-10 flex flex-col md:flex-row items-start md:items-center justify-between gap-6">
          ...
          <div className="flex flex-col sm:flex-row md:flex-col gap-2.5 w-full md:w-auto shrink-0">
            <button
              type="button"
              onClick={() => {
                setTargetDeptForReport(null);
                setIsLeaderGuardOpen(true);
              }}
              className="flex items-center justify-center gap-2 px-5 py-3 rounded-2xl bg-white text-[#0A3D36] hover:bg-amber-50 font-black text-xs sm:text-sm shadow-lg transition-all hover:scale-102 active:scale-95 cursor-pointer border border-[#C59A27]"
              title="Espace sécurisé pour les responsables de département (mot de passe requis)"
            >
              <Lock className="w-4 h-4 text-[#C59A27]" />
              <span>Espace Responsable & Rapports 🔒</span>
            </button>
        ...
        <div className="fixed inset-0 z-50 bg-black/65 backdrop-blur-xs flex items-center justify-center p-3 sm:p-4 overflow-y-auto">
          <div className="bg-white rounded-3xl w-full max-w-3xl my-6 p-5 sm:p-8 shadow-2xl border border-slate-100 text-slate-800 space-y-6">
              ...
              <div className="p-4 rounded-2xl bg-gradient-to-br from-slate-50 to-amber-50/40 border border-amber-200 flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
                ...
                <div className="flex items-center gap-2 w-full sm:w-auto flex-wrap">
                  <button
                    type="button"
                    onClick={() => {
                      setTargetDeptForReport(selectedDept);
                      setIsLeaderGuardOpen(true);
                    }}
                    className="flex-1 sm:flex-none flex items-center justify-center gap-1.5 px-3.5 py-2 rounded-xl bg-[#C59A27] text-slate-950 hover:bg-[#d4a936] text-xs font-black transition-all shadow-xs"
                    title="Rédiger et envoyer un rapport de ce département au Pasteur (code secret requis)"
                  >
            ...
            <div className="space-y-3 pt-2 border-t border-slate-100">
              ...
              {/* Suivi des Présences au Culte dans le Département */}
              <div className="bg-slate-50 p-3.5 rounded-2xl border border-slate-200/80 space-y-3">
                <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-2">
                  <div className="flex items-center gap-1.5 text-xs font-black text-[#0A3D36]">
                    <Calendar className="w-3.5 h-3.5 text-[#C59A27]" />
                    <span>Pointage Dominical • Culte ({deptPresenceStatus.presents.length}/{selectedDept.membersList?.length || 0} présents)</span>
                  </div>

                  <div className="flex items-center gap-2">
                    <span className="text-[11px] font-bold text-slate-500">Dimanche :</span>
                    <select
                      value={selectedSunday}
                      onChange={(e) => setSelectedSunday(e.target.value)}
                      className="px-2.5 py-1 rounded-xl border border-slate-300 text-xs font-bold bg-white text-slate-900 focus:ring-1 focus:ring-[#0A3D36]"
                    >
                      {allSundays.map((d) => (
                ...
                {/* Sub-tabs: Tous / Présents / Absents */}
                <div className="flex items-center gap-2">
                  <button
                    type="button"
                  ...
                  >
                    Tous ({selectedDept.membersList?.length || 0})
                  </button>
                  ...
                  >
                    ...
                    <span>Présents ({deptPresenceStatus.presents.length})</span>
                  </button>
                  ...
                  >
                    ...
                    <span>Absents ({deptPresenceStatus.absents.length})</span>
                  </button>
                </div>
              </div>
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-2.5 max-h-64 overflow-y-auto pr-1">
                {filteredDeptMembers
                  .filter((m) => {
                    if (presenceSubTab === 'presents') {
                      return deptPresenceStatus.presents.some((p) => p.id === m.id);
                    }
                    if (presenceSubTab === 'absents') {
                      return deptPresenceStatus.absents.some((a) => a.id === m.id);
                    }
                    return true;
                  })
                  .map((member) => {
                    const isPresentThisSunday = deptPresenceStatus.presents.some((p) => p.id === member.id);
                    const matchedRecord = deptPresenceStatus.matchedRecordsMap.get(member.id);
                      ...
                      >
                        <div className="flex items-center gap-2.5 min-w-0">
                          <div className="relative shrink-0">
                            ...
                            {isPresentThisSunday ? (
                              <span
                                className="absolute -top-1 -right-1 w-3.5 h-3.5 bg-emerald-500 rounded-full border-2 border-white flex items-center justify-center"
                                title={`Présent(e) au culte du dimanche ${selectedSunday}`}
                              >
                                <CheckCircle2 className="w-2.5 h-2.5 text-white" />
                              </span>
                            ) : (
                              <span
                                className="absolute -top-1 -right-1 w-3.5 h-3.5 bg-rose-500 rounded-full border-2 border-white flex items-center justify-center"
                                title={`Non pointé(e) au culte du dimanche ${selectedSunday}`}
                              >
                                <UserX className="w-2 text-white" />
                              </span>
                            )}
                          </div>
      ...
      {/* Leadership Access Guard for Department Leader */}
      {isLeaderGuardOpen && (
        <LeadershipGuardModal
          category="CHEF_DEPARTEMENT"
          title={`Espace Sécurisé • Responsable ${targetDeptForReport ? targetDeptForReport.name : 'de Département'}`}
          targetName={targetDeptForReport ? targetDeptForReport.name : 'Départements'}
      ...
      {isCreateReportOpen && (
        <CreateSendReportModal
          category="CHEF_DEPARTEMENT"
          defaultEntityName={targetDeptForReport?.name || activeLeaderAccount?.targetName || 'Département de Louange & Adoration'}
          defaultLeaderName={activeLeaderAccount?.holderName || targetDeptForReport?.leaderName || ''}
          defaultLeaderPhone={activeLeaderAccount?.phone || targetDeptForReport?.leaderPhone || ''}
          templates={rapportTemplates}
          currentUser={currentUser}
          onClose={() => setIsCreateReportOpen(false)}
          onSubmitReport={(report) => {
            if (onSubmitReport) onSubmitReport(report);
            showToast('Rapport soumis et transmis au Pasteur avec succès !');
          }}
        />
      )}
    </div>
