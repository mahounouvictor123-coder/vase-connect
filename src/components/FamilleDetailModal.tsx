...
import { FamilleHonneur, FamilleHonneurInscription, FamilleReunionPhoto, UserProfile, CultePresenceRecord } from '../types';
...
export const FamilleDetailModal: React.FC<FamilleDetailModalProps> = ({
  ...
  // Sunday presence tracking in Famille d'Honneur
  const allSundays = React.useMemo(() => getAllSundayDates(presences), [presences]);
  ...
  const familyPresenceStatus = React.useMemo(() => {
    if (familyInscriptions.length === 0) {
      return { presents: [], absents: [], chronicAbsents: [], presenceRatio: 0, matchedRecordsMap: new Map() };
    }
    return computeCompartmentPresence(familyInscriptions, presences, selectedSunday, allSundays);
  }, [familyInscriptions, presences, selectedSunday, allSundays]);
        ...
        <div className="p-4 sm:p-6 overflow-y-auto space-y-5 flex-1">
          ...
          {activeTab === 'inscription' && (
            <div className="space-y-5">
              ...
              {/* Roster of members registered in this family with Sunday Presence Tracking */}
              <div className="space-y-3 pt-2 border-t border-slate-200">
                <div className="bg-slate-50 p-3 rounded-2xl border border-slate-200 space-y-2.5">
                  <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-2">
                    <div className="flex items-center gap-1.5 text-xs font-black text-[#0A3D36]">
                      ...
                      <span>Pointage Dominical • Culte ({familyPresenceStatus.presents.length}/{familyInscriptions.length} présents)</span>
                    </div>
                  ...
                  <div className="flex items-center gap-2">
                    ...
                    >
                      Tous ({familyInscriptions.length})
                    </button>
                    ...
                    >
                      ...
                      <span>Présents ({familyPresenceStatus.presents.length})</span>
                    </button>
                    ...
                    >
                      ...
                      <span>Absents ({familyPresenceStatus.absents.length})</span>
                    </button>
                  </div>
                </div>
                ...
                <div className="flex items-center justify-between">
                  <h4 className="text-xs font-black uppercase text-slate-600 tracking-wider">
                    Membres ({familyInscriptions.length} enregistrés)
                  </h4>
                ...
                {familyInscriptions.length > 0 ? (
                    ...
                    {familyInscriptions
                      .filter((member) => {
                        if (presenceSubTab === 'presents') {
                          return familyPresenceStatus.presents.some((p) => p.id === member.id);
                        }
                        if (presenceSubTab === 'absents') {
                          return familyPresenceStatus.absents.some((a) => a.id === member.id);
                        }
                        return true;
                      })
                      .map((member, idx) => {
                        const isPresent = familyPresenceStatus.presents.some((p) => p.id === member.id);
                        const matched = familyPresenceStatus.matchedRecordsMap.get(member.id);

                        return (
                          <div
                            key={member.id || idx}
                            className={`p-2.5 rounded-xl border flex items-center justify-between text-xs transition-colors ${
                              isPresent ? 'bg-emerald-50/40 border-emerald-200' : 'bg-slate-50 border-slate-100'
                            }`}
                          >
                  ...
                  <p className="text-xs text-slate-500 italic p-3 bg-slate-50 rounded-xl">
                    Soyez parmi les premiers membres inscrits pour cette famille d'honneur via le formulaire ci-dessus.
                  </p>
                )}
              </div>
            </div>
